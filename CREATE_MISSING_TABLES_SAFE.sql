-- Phase 1 Strategic UX: Create Missing Tables (Safe Version)
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'linkedin_posts_author_id_fkey') THEN
        ALTER TABLE public.linkedin_posts 
        ADD CONSTRAINT linkedin_posts_author_id_fkey 
        FOREIGN KEY (author_id) REFERENCES public.users(id) ON DELETE CASCADE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'linkedin_posts_ai_generation_id_fkey') THEN
        ALTER TABLE public.linkedin_posts 
        ADD CONSTRAINT linkedin_posts_ai_generation_id_fkey 
        FOREIGN KEY (ai_generation_id) REFERENCES public.ai_generations(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'linkedin_posts_source_post_id_fkey') THEN
        ALTER TABLE public.linkedin_posts 
        ADD CONSTRAINT linkedin_posts_source_post_id_fkey 
...
-- 4. Leads & Collaborations - Create ENUMs first
DO $$ BEGIN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_leads_status_created') THEN
        CREATE INDEX idx_leads_status_created ON public.leads(status, created_at);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_leads_email') THEN
        CREATE INDEX idx_leads_email ON public.leads(email);
    END IF;
END $$;

-- 5. Case Studies - Create ENUM first
DO $$ BEGIN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'case_studies_author_id_fkey') THEN
        ALTER TABLE public.case_studies 
        ADD CONSTRAINT case_studies_author_id_fkey 
        FOREIGN KEY (author_id) REFERENCES public.users(id) ON DELETE CASCADE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'case_studies_featured_image_id_fkey') THEN
        ALTER TABLE public.case_studies 
        ADD CONSTRAINT case_studies_featured_image_id_fkey 
...
-- 6. AI Configuration - Create ENUMs first
DO $$ BEGIN
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migration complete! All Phase 1 tables created successfully!';
    
    -- Report which tables were created
    RAISE NOTICE 'Tables created: site_logos, linkedin_posts, subscribers, leads, case_studies, ai_configs, ai_prompt_templates';
END $$;
